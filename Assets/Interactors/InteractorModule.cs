...
namespace Interactors
{
    ...
    public abstract class InteractorModule : MonoBehaviour
    {
        ...
        {
            var resources = block.GetConnectedPlanet().GetResources();
            if (resources.GetOre() < costs.ore) return new Tuple<TinyPlanetResources.PlanetResourceType, int>(TinyPlanetResources.PlanetResourceType.Ore, costs.ore);
            if (resources.GetMetals() < costs.metals) return new Tuple<TinyPlanetResources.PlanetResourceType, int>(TinyPlanetResources.PlanetResourceType.Metals, costs.metals);
            if (resources.GetGadgets() < costs.gadgets) return new Tuple<TinyPlanetResources.PlanetResourceType, int>(TinyPlanetResources.PlanetResourceType.Gadgets, costs.gadgets);
